...
public:
    ...
    Camera(glm::vec3 front=glm::vec3(0.0f, 0.0f, -1.0f),glm::vec3 position = glm::vec3(0.0f, 0.0f, 3.0f), glm::vec3 up = glm::vec3(0.0f, 1.0f, 0.0f), float yaw = YAW, float pitch = PITCH) :  MovementSpeed(SPEED), MouseSensitivity(SENSITIVITY), Zoom(ZOOM)
    {
        Front = front ;
        Position = position;
        WorldUp = up;
